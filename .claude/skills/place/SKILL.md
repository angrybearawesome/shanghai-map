...
사용자가 여행 중 UI로 추가한 '내 장소'(localStorage `shcafemap.uplaces.v1`, uid는 `u...`)를
정식 카드로 올리는 작업. 사용자가 "내 장소 ○○ 정식 등록해줘"라고 하거나 계획 공유
링크(v3 — base64url 디코드하면 `{v:3, d:..., u:[{i,n,g,la,lo,m}]}`)를 붙여넣는다.

1. 링크/설명에서 이름·그룹·GCJ-02 좌표(지도에서 찍은 값)·메모를 얻는다
2. **일반 추가 절차의 리서치를 그대로 수행** — 사용자가 찍은 좌표는 출발점일 뿐,
   정식 중문 명칭·주소·좌표 교차 검증을 다시 한다 (찍은 좌표와 리서치 좌표가
   수백 m 이상 다르면 사용자에게 확인)
3. 새 번호로 정식 카드 삽입 (이하 추가 절차와 동일: 검증→커밋→배포)
4. 보고에 명시: **uid로 쌓인 방문 기록은 새 번호로 자동 이관되지 않는다** —
   사용자가 원하면 localStorage에서 visits.v2의 해당 uid를 새 번호로 치환하는
   방법을 안내 (개발자 콘솔 한 줄이면 됨)
5. 사용자에게 해당 내 장소를 삭제하라고 안내 (중복 핀 방지)
